...
function main()
	...
	ui.setMapName("Snake by Themiachale\n")
	add(-3, "<a href='event:play'>Gioca</a>", nil, 755, 375)
...
function eventNewPlayer(name)
	if gamemode=="Classic" then
		...
		if isPlaying then 
			...
			ui.setMapName("Snake by Themiachale <font color='#6A7495'>|</font> <font color='#C2C2DA'>Lunghezza: "..len.."</font>\n")
		end
...
function eventNewGame()
	...
	pressedKey = RIGHT
	len = 7
	tfm.exec.setUIShamanName("Serpente: ("..shamanName..")")
	...
	timerTextArea(99, "<font size='12'>Sei lo sciamano! Salva i topi muovendo il serpente.</font>", shamanName, 225, 370, 4, nil, nil, 0x1b232a,0x000000)
...
function eventTextAreaCallback(id, name, event)
	...
	if event=="play" and gamemode~="SS" then
		...
		if name~=playerList[1] and not gameover then --Is name the current player?
			for i, k in pairs(playerList) do
				if k==name then --Is name in the player list?
					timerTextArea(100, "Sei già in coda, sei nella posizione: "..i..".", name, 275, 300, 5, nil, nil, 0x1b232a,0x000000)
					return
				end
			end
			table.insert(playerList, name) --name isn't the current player nor is in the list so ill save his name
			timerTextArea(101, "Sei stato aggiunto alla coda, sei nella posizione: "..#playerList..".", name, 275, 327, 5, nil, nil, 0x1b232a,0x000000)
		elseif gameover then --Is the game over textarea showing?
			...
			timerTextArea(103, "Stai già giocando!", name, 275, 378, 5, nil, nil, 0x1b232a,0x000000)
		end
	elseif event=="SS" and gamemode=="" then
		for _ in pairs(tfm.get.room.playerList) do
			p = p + 1
		end
		if p <= 2 then ui.addPopup (0, 0, "Servono almeno 3 topi per giocare questa modalità.", admin, 275, 304); return; end
	...
	elseif gamemode=="SS" and event=="play" and name==admin then
		ui.addPopup (1, 1, "Sei sicuro di voler uscire dalla modalità SnakeShaman?", admin, 300, 200)
...
function gamemodeClassic(reset)
	...
	timerTextArea(99, "<font size='12'>Sta giocando: "..playerList[1].."</font>", nil, 275, 273, 6, nil, nil, 0x1b232a,0x000000)
...
function classicLoop()
	...
	elseif tick==0 and gameover then
		...
		gameover = false
		foodMov = 0
		ui.setMapName("Snake by Themiachale <font color='#6A7495'>|</font> <font color='#C2C2DA'>Aspettando un giocatore... (Clicca su 'Gioca' per partecipare)</font>\n")
