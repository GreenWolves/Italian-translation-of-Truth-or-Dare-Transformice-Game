...
function start_new_game()
	...
	output_msg(OUTPUT_NAME_COLOR .. ice_mouse .. OUTPUT_MESSAGE_COLOR .. ' è il topo congelante!')
	output_msg(OUTPUT_MESSAGE_COLOR .. 'Il round inizierà in 10 secondi!')
	countdown = 10
end
...
tfm.exec.setUIMapName("(M)Ice tag by Themiachale")
	...
	if second_mark then
		...
		elseif countdown == 0 then
			countdown_msg(OUTPUT_MESSAGE_COLOR .. 'Via!')
			countdown = -1
			can_attack = true
		elseif countdown == 20 then
			output_msg(OUTPUT_MESSAGE_COLOR .. 'Startando un nuovo round!')
			start_new_game()
		elseif countdown > 20 then
			countdown = countdown - 1
		end
	...
	if time_remaining <= 1 then
		...
		output_msg(OUTPUT_MESSAGE_COLOR .. 'Oh no! ' .. OUTPUT_NAME_COLOR .. ice_mouse .. OUTPUT_MESSAGE_COLOR .. ' non è riuscito a congelare tutti i topi!')
		output_msg(OUTPUT_MESSAGE_COLOR .. 'Startando un nuovo round!')
		start_new_game()
	end
...
function check_alive()
	...
	if all_dead then
		...
		output_msg(OUTPUT_NAME_COLOR .. ice_mouse .. OUTPUT_MESSAGE_COLOR .. ' ha congelato tutti i topi!')
		output_msg(OUTPUT_NAME_COLOR .. ice_mouse .. OUTPUT_MESSAGE_COLOR .. ' vince!')
		output_msg(OUTPUT_MESSAGE_COLOR .. 'Il prossimo round inizierà in 5 secondi')
...
function eventPlayerDied(name)
	if name == ice_mouse then
		...
		output_msg(OUTPUT_MESSAGE_COLOR .. '' .. OUTPUT_NAME_COLOR .. ice_mouse .. OUTPUT_MESSAGE_COLOR .. ' è morto!')
		output_msg(OUTPUT_MESSAGE_COLOR .. 'Il prossimo round inizierà in 5 secondi')
		set_new_game_countdown(5)
	else
		output_msg(OUTPUT_NAME_COLOR .. name .. OUTPUT_MESSAGE_COLOR .. ' è stato congelato!')
...
function search_and_rescue(rescuee)
	for id in pairs(ice_blocks) do
		if (math.abs(tfm.get.room.objectList[id].x - tfm.get.room.playerList[rescuee].x) <= RESCUE_RADIUS) and (math.abs(tfm.get.room.objectList[id].y - tfm.get.room.playerList[rescuee].y) <= RESCUE_RADIUS) then
			output_msg(OUTPUT_NAME_COLOR .. ice_blocks[id] .. OUTPUT_MESSAGE_COLOR .. ' è stato scongelato da ' .. OUTPUT_NAME_COLOR .. rescuee)
